...
export default function SolutionsList({ onAdd, onEdit, notice }: Props) {
  ...
  async function load(nextQuery: string, nextPage: number) {
    setError(null);
    try {
      applyPagedResult(await listDigitalTransformations(nextQuery, nextPage), nextPage, setPage, setData);
    } catch {
  ...
  const bulk = useBulkTable({
    ...
    resetKey: `/admin/digital-transformation:${page}:${appliedQuery}`,
    path: '/admin/digital-transformation',
