...
from flycatch_api.models import CloudService as BulkModel
from flycatch_api.models.cloud_service import CloudService
from flycatch_api.schemas.admin_named_pages import CloudService as CloudServiceSchema
from flycatch_api.schemas.admin_named_pages import CloudServiceList, CloudServiceWrite
...
router = APIRouter(prefix="/admin/cloud-services", tags=["admin-cloud-services"])
...
@router.get("", response_model=CloudServiceList)
...
@router.post("", response_model=CloudServiceSchema, status_code=status.HTTP_201_CREATED)
def create_cloud_service(
    payload: CloudServiceWrite, session: CurrentSession, db: Session = Depends(get_db)
):
    assert_write_permissions(
...
@router.get("/{entry_id}", response_model=CloudServiceSchema)
def get_cloud_service(entry_id: UUID, session: CurrentSession, db: Session = Depends(get_db)):
    assert_resource_action(db, session.administrator_id, RESOURCE, "read")
    try:
...
@router.patch("/{entry_id}", response_model=CloudServiceSchema)
def update_cloud_service(
    entry_id: UUID,
    payload: CloudServiceWrite,
    session: CurrentSession,
    db: Session = Depends(get_db),
):
    assert_write_permissions(
...
def delete_cloud_service(entry_id: UUID, session: CurrentSession, db: Session = Depends(get_db)):
...
attach_bulk_routes(router, resource=RESOURCE, model=BulkModel, not_found_key='admin.cloud_services.not_found')
