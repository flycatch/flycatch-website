...
def create_client_testimonial(
    payload: ClientTestimonialWrite, _session: RequireDraft, db: Session = Depends(get_db)
):
    try:
...
def get_client_testimonial(
    testimonial_id: UUID, _session: RequireView, db: Session = Depends(get_db)
):
    try:
...
def delete_client_testimonial(
    testimonial_id: UUID, _session: RequireDraft, db: Session = Depends(get_db)
):
    try:
