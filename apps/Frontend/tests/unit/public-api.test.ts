...
import {
  ...
  loadPublishedInfrastructureManagement,
  loadPublishedInfrastructureManagements,
...
describe('public devops consult loaders', () => {
  it('lists published devops consult pages from the public API', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        items: [
          {
            slug: 'devops-consultation',
            banner_title: 'DevOps Consulting Company',
            banner_image_key: null,
          },
        ],
        page: 1,
    ...
    vi.stubGlobal('fetch', fetchMock);
    const result = await loadPublishedDevOpsConsults();
    expect(result.error).toBe(false);
    expect(result.items[0].slug).toBe('devops-consultation');
    expect(String(fetchMock.mock.calls[0][0])).toContain('/api/v1/public/devops-consult');
    vi.unstubAllGlobals();
  });
  ...
  it('loads published detail by slug without substituting designed content', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        slug: 'devops-consultation',
        banner_title: 'CMS banner',
        banner_image_key: 'devops/hero.jpg',
        introduction_title: 'CMS intro',
        introduction_first_paragraph: 'First',
        introduction_second_paragraph: 'Second',
        experience_title: 'CMS tools',
        experience_accordion: [{ title: 'CI', contents: '<p>Body</p>', order: 0 }],
        experience_image_key: 'devops/tools.jpg',
        experience_description: '<p>Tools</p>',
        faq_title: '',
        faq_description: '',
        faq_accordion: [],
        seo: {
          title: 'DevOps Consulting Company',
          description: '',
          canonical_url: '/services/devOps-consultation',
          meta_title: '',
          h1_tag: '',
          image_alt: '',
          image_key: null,
    ...
    vi.stubGlobal('fetch', fetchMock);
    const result = await loadPublishedDevOpsConsult('devops-consultation');
    expect(result.error).toBe(false);
    expect(result.item?.banner_title).toBe('CMS banner');
    expect(result.item?.banner_image_key).toBe('devops/hero.jpg');
    expect(result.item?.experience_title).toBe('CMS tools');
    expect(result.item?.experience_accordion).toHaveLength(1);
    expect(String(fetchMock.mock.calls[0][0])).toContain(
      '/api/v1/public/devops-consult/devops-consultation',
    );
    vi.unstubAllGlobals();
  });
});

describe('public infrastructure management loaders', () => {
  it('lists published infrastructure management pages from the public API', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        items: [
          {
            slug: 'infrastructure-management-and-automation',
            banner_title: 'Infrastructure Management & Automation Services',
            banner_image_key: null,
          },
        ],
        page: 1,
    ...
    vi.stubGlobal('fetch', fetchMock);
    const result = await loadPublishedInfrastructureManagements();
    expect(result.error).toBe(false);
    expect(result.items[0].slug).toBe('infrastructure-management-and-automation');
    expect(String(fetchMock.mock.calls[0][0])).toContain(
  ...
  it('loads published detail by slug without substituting designed content', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        slug: 'infrastructure-management-and-automation',
        banner_title: 'CMS banner',
        banner_image_key: 'infra/hero.jpg',
        introduction_title: 'CMS intro',
        introduction_first_paragraph: 'First',
        introduction_second_paragraph: 'Second',
        faq_title: '',
        ...
        faq_accordion: [],
        seo: {
          title: 'Infrastructure management & automation services',
          description: '',
          canonical_url: '/services/infrastructure-management-and-automation',
          meta_title: '',
          h1_tag: '',
          image_alt: '',
          image_key: null,
    ...
    vi.stubGlobal('fetch', fetchMock);
    const result = await loadPublishedInfrastructureManagement(
      'infrastructure-management-and-automation',
    );
    expect(result.error).toBe(false);
    expect(result.item?.banner_title).toBe('CMS banner');
    expect(result.item?.banner_image_key).toBe('infra/hero.jpg');
    expect(result.item?.introduction_first_paragraph).toBe('First');
    expect(String(fetchMock.mock.calls[0][0])).toContain(
      '/api/v1/public/infrastructure-management/infrastructure-management-and-automation',
    );
    vi.unstubAllGlobals();
  });
});
